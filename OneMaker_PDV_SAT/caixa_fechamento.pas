...
var
  codOperador:string;
  dValor, dSangria, dSuprimento, dTotal, dTroco, dpromissoria, dDinheiro:double;
begin
  dValor := 0;  dSangria := 0;  dSuprimento := 0;  dTotal := 0; dDinheiro:=0; dTroco := 0; dpromissoria :=0;
   qrFechamento.close;
  ...
  qrFechamento.sql.add
    ('    (select info1 from adm where codigo = cupom.COD_VENDEDOR) as Operador,');
  qrFechamento.sql.add('    cupom_forma.FORMA as forma,');
  qrFechamento.sql.add('    cupom_forma.VALOR as valor');
  qrFechamento.sql.add('  from');
  qrFechamento.sql.add
    ('    cupom inner join cupom_forma on cupom_forma.COD_CUPOM = cupom.CODIGO');
  qrFechamento.sql.add('    where  cupom.DATA + cupom.Hora >= :data');
  qrFechamento.sql.add('union all');
  qrFechamento.sql.add('  select');
  qrFechamento.sql.add('    nao_fiscal.CODVENDEDOR as CodOperador,');
  qrFechamento.sql.add
    ('    (select info1 from adm where codigo = nao_fiscal.CODVENDEDOR) as Operador,');
  ...
  qrFechamento.sql.add
    ('  where nao_fiscal.Data + nao_fiscal.hora >= :data and nao_fiscal.INDICE <> '
    + QuotedStr('RG'));
  ...
  begin
    ...
    GridFechamento.Cell[0, GridFechamento.LastAddedRow].asinteger :=
      qrFechamento.fieldbyname('codoperador').Value;
    GridFechamento.Cell[1, GridFechamento.LastAddedRow].asstring :=
      qrFechamento.fieldbyname('operador').asstring;

    if qrFechamento.fieldbyname('forma').asstring = 'SANGRIA' then
    begin
      GridFechamento.Cell[2, GridFechamento.LastAddedRow].asstring :=
        qrFechamento.fieldbyname('forma').asstring + ' (-)';
      GridFechamento.Cell[3, GridFechamento.LastAddedRow].asstring :=
        FormatarValor(qrFechamento.fieldbyname('total').asfloat * (-1),
        2, false);
    ...
    begin
      GridFechamento.Cell[2, GridFechamento.LastAddedRow].asstring :=
        qrFechamento.fieldbyname('forma').asstring + ' (+)';
      GridFechamento.Cell[3, GridFechamento.LastAddedRow].asstring :=
        FormatarValor(qrFechamento.fieldbyname('total').asfloat, 2, false);
      dSuprimento := dSuprimento + qrFechamento.fieldbyname('total').asfloat;
    end
    else if qrFechamento.fieldbyname('forma').asstring = 'Dinheiro' then
    begin
      GridFechamento.Cell[2, GridFechamento.LastAddedRow].asstring :=
        qrFechamento.fieldbyname('forma').asstring;
      GridFechamento.Cell[3, GridFechamento.LastAddedRow].asstring :=
        FormatarValor(qrFechamento.fieldbyname('total').asfloat - dTroco,
        2, false);
  ...
  begin
    ...
    GridFechamento.Cell[3, GridFechamento.LastAddedRow].asstring :=
      FormatarValor(dValor + dSuprimento - dSangria - dTroco, 2, false);
  ...
  qrFechamento.sql.add
    (' cupom inner join cupom_forma on cupom_forma.COD_CUPOM = cupom.CODIGO');
  qrFechamento.sql.add
    ('  where cupom.DATA + cupom.Hora >= :data and (cupom_forma.Forma =''Dinheiro'' or cupom_forma.Forma =''DEVOLUÇÃO'')');
  qrFechamento.sql.add('union all');
  qrFechamento.sql.add('  select');
  qrFechamento.sql.add('    nao_fiscal.DESCRICAO as forma,');
  qrFechamento.sql.add('    nao_fiscal.VALOR as valor');
  ...
  GridFechamento.Cell[2, GridFechamento.LastAddedRow].asstring :=
    'TOTAL EM DINHEIRO';
  GridFechamento.Cell[3, GridFechamento.LastAddedRow].asstring :=
    FormatarValor(dDinheiro - dTroco, 2, false);
  GridFechamento.Cell[2, GridFechamento.LastAddedRow].FontStyle := [fsBold];
  GridFechamento.Cell[3, GridFechamento.LastAddedRow].FontStyle := [fsBold];
  ...
  begin
    ...
    else if qrFechamento.fieldbyname('forma').asstring = 'Promissoria' then
    begin
      GridFechamento.Cell[2, GridFechamento.LastAddedRow].asstring :=
        qrFechamento.fieldbyname('forma').asstring;;
      GridFechamento.Cell[3, GridFechamento.LastAddedRow].asstring :=
        FormatarValor(qrFechamento.fieldbyname('total').asfloat, 2, false);
      dpromissoria := dpromissoria + qrFechamento.fieldbyname('total').asfloat;
    end
    else
    begin
      GridFechamento.Cell[2, GridFechamento.LastAddedRow].asstring :=
  ...
  GridFechamento.Cell[3, GridFechamento.LastAddedRow].asstring :=
    FormatarValor(dTotal - dSangria - dpromissoria - dTroco, 2, false);
  GridFechamento.Cell[2, GridFechamento.LastAddedRow].FontStyle := [fsBold];
  GridFechamento.Cell[3, GridFechamento.LastAddedRow].FontStyle := [fsBold];
