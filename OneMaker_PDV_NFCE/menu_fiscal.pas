...
uses Leitura_Memoria_Fiscal, Download_MFD, Arquivo_Fiscal,
  ...
  msg_Operador, Lista_Vendas_Periodo, tabela_indice, Constantes, UFuncoes;
...
begin
  ...
  begin
    ...
    application.messagebox(pwidechar('Erro na emissão da Leitura X'+
...
begin
  ...
  DARUMA:
    begin
      ...
      begin
        application.MessageBox(pwidechar(sMsg),'Aviso',mb_ok+MB_ICONINFORMATION);

      end
      else
      begin
        application.MessageBox(pwidechar(sMsg),'Erro',mb_ok+mb_iconerror);
...
begin
  sNome_Arquivo := '.\txt\TabProd.txt';

  if FileExists(SystemDrive + '\TabProd.txt') then DeleteFile(SystemDrive +'\TabProd.txt');
