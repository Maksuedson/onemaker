...
uses
  ...
  NxCustomGridControl, NxCustomGrid, NxGrid, AdvReflectionLabel, DB, DBAccess,
  TFlatPanelUnit, Menus, Buttons, RzPanel, AdvGlowButton,
  RzCmboBx, AdvCombo, TFlatComboBoxUnit, ACBrBase, ACBrBAL, ACBrDIS,
  AdvMenus, principal, FileCtrl, FlCtrlEx, ImgList,pcnAuxiliar, pcnConversao,
  ActnList, XPStyleActnCtrls, ActnMan, pngimage, Clipbrd, AdvPicture, AdvGDIPicture,
  AdvReflectionImage, acPNG,CPort, System.Actions, ResizeKit, dxGDIPlusClasses,
  AdvTimePickerDropDown, IdBaseComponent, IdComponent, IdIPWatch,
  Uni, MemDS,ACBrUtil,ACBrEAD, pcnNFeW, pcnConversaoNFe, Vcl.DBCtrls,
  AdvSmoothPanel;
...
type
  TfrmVenda = class(TForm)
    ...
    QRCSOSN: TUniQuery;
    SQL_ItemCcancelado: TUniQuery;
    query_emitente: TUniQuery;
    query_emitenteCODIGO: TStringField;
    query_emitenteNOME: TStringField;
    query_emitenteFANTASIA: TStringField;
    query_emitenteENDERECO: TStringField;
    query_emitenteBAIRRO: TStringField;
    query_emitenteCIDADE: TStringField;
    query_emitenteUF: TStringField;
    query_emitenteCEP: TStringField;
    query_emitenteTELEFONE: TStringField;
    query_emitenteCNPJ: TStringField;
    query_emitenteIE: TStringField;
    query_emitenteEMAIL: TStringField;
    query_emitenteHOMEPAGE: TStringField;
    query_emitenteRESPONSAVEL: TStringField;
    query_emitenteCELULAR: TStringField;
    query_emitenteLOGO: TBlobField;
    query_emitenteFAX: TStringField;
    query_emitenteNUMERO: TStringField;
    query_emitenteCOMPLEMENTO: TStringField;
    query_emitenteCONTRIBUINTE_IPI: TStringField;
    query_emitenteSUBSTITUTO_TRIBUTARIO: TStringField;
    query_emitenteCOMENTARIOS: TStringField;
    query_emitenteDATAHORA_INICIAL: TDateField;
    query_emitenteDATAHORA_FINAL: TDateField;
    query_emitenteDATA_INVENTARIO: TDateField;
    query_emitenteESTOQUE: TFloatField;
    query_emitenteVALOR: TFloatField;
    query_emitenteCONHECIMENTO: TIntegerField;
    query_emitenteCPF: TStringField;
    query_emitenteRG: TStringField;
    query_emitenteDIA_VENCIMENTO_CHAVE: TStringField;
    query_emitenteINSC_MUNICIPAL: TStringField;
    query_emitenteDATA_ABERTURA: TDateField;
    query_emitenteCOD_MUNICIPIO_IBGE: TStringField;
    query_emitenteIBGE: TStringField;
    query_emitenteEMAIL_FINANCEIRO: TStringField;
    DataSource_emitente: TDataSource;
    Panel7: TPanel;
    Image2: TImage;
  ...
  public
    ...
    function ItemCancelado(item:integer):boolean;
  end;
...
begin
      ...
      fl.Directory := '\siscom\paf\dat\carga';
      fl.Refresh;
      fl.Update;
    ...
    begin
      ...
      begin
        ...
        // lancar a forma de pagamento - DINHEIRO
          ...
          if frmPrincipal.TipoImpressora = Fiscal then
            spCupom_Forma.ParamByName('tipo').asstring := 'CUPOM NFCe'
          else
...
function TfrmVenda.ItemCancelado(item: integer): boolean;
begin
  with SQL_ItemCcancelado do
  begin
    close;
    sql.Clear;
    sql.add('select codigo from CUPOM_ITEM_TEMP where item='+item.tostring+' and cod_cupom='''+ sNumero_Cupom+'''');
    open;
    result := recordcount =2;
//    if copy(SQL_ItemCcancelado.fieldbyname('codigo').AsString,length(SQL_ItemCcancelado.fieldbyname('codigo').AsString)-1,1)='C' then
...
//     xcpf.Caption :='CPF:' + sConsumidor_CPF;
    ...
    begin
      ...
      grid_topo.AddRow(1);
      grid_topo.cell[0, grid_topo.LastAddedRow].asstring :=
        '<B><FONT size="12">             CUPOM NFCe</FONT></B>';
      grid_topo.AddRow(1);
      grid_topo.cell[0, grid_topo.LastAddedRow].asstring :=
...
begin
  ...
  begin
    ...
    frmmodulo.cdsComandas.CreateDataSet; // Caso esteja fechado
...
begin
  ...
  begin
    ...
    pn_fechamento.Top := 84;
    pn_fechamento.Left := 184;
        ...
        begin
          ...
          begin
            if edtPathLogs <> '' then
              ...
              NomeArquivo := 'C:\OneMaker\PDV\xml';
            ...
            if vContingencia=0 then
            spNFCE_Insert.ParamByName('psituacao').asinteger := 0 else
            spNFCE_Insert.ParamByName('psituacao').asinteger := 999;
...
var
  ...
  Imposto_Nac, Imposto_Est ,  Imposto_Imp :real;
  Total_Imp_Nac,Total_Imp_Est,Total_Imp_Importacao:real;
begin
  try
    with frmmodulo do
    begin
      ...
      begin
        ...
        if ACBRNFCe.WebServices.StatusServico.cStat <> 107 then
        begin
          ACBRNFCe.Configuracoes.Geral.FormaEmissao := teOffLine;
          vContingencia := 1;
          Ide.tpEmis := teOffLine;
          Ide.xJust := 'PROBLEMAS POR FALTA DE CONEXAO COM INTERNET';
          Ide.dhCont := Now;
        end;
        ...
        Total_Imp_Nac      :=0;
        Total_Imp_Est      :=0;
        Total_Imp_Importacao:=0;

        for i := 0 to grid.RowCount - 1 do
        begin


          if (grid.Cell[1, i].asinteger = 1)and (not ItemCancelado( grid.cell[2, i].asinteger)) then
          begin
             // prepar query
            qrProdNCFE.close;
            ...
            begin
              ...
              Prod.vUnCom := RoundTo(grid.Cell[6, i].asfloat,-2); // miza
              Prod.vUnTrib := RoundTo(grid.Cell[6, i].asfloat,-2);
              ...
              begin
                ...
                SQL_Exec.Close;
                SQL_Exec.sql.Clear;
                SQL_Exec.SQL.Add('select * from ncm_uf where id=''' +somenteNumero(qrProdNCFE.fieldbyname('NCM').asstring)+ '''');
                SQL_Exec.Open;
                if SQL_Exec.RecordCount = 0 then
                begin
                  Imposto_Nac := 0;
                  Imposto_Est := 0;
                  Imposto_Imp := 0;
                ...
                begin
                  if SQL_Exec.FieldByName('nacionalfederal').AsFloat > 0 then
                  begin
                    Imposto_Nac := RoundTo(RoundTo(grid.Cell[5, i].asfloat/100,-2) / 100 * SQL_Exec.FieldByName('nacionalfederal').AsFloat, -2);
                  end
                  else
                  begin
                    Imposto_Nac := 0;
                  ...
                  //Importado Federal
                  if SQL_Exec.FieldByName('importadosfederal').AsFloat > 0 then
                  begin
                    Imposto_Imp := RoundTo(RoundTo(grid.Cell[5, i].asfloat/100,-2) / 100 * SQL_Exec.FieldByName('importadosfederal').AsFloat, -2);
                  end
                  else
                  begin
                    Imposto_Imp := 0;
                  ...
                  //Estadual
                  if SQL_Exec.FieldByName('imposto_estadual').AsFloat > 0 then
                  begin
                    Imposto_Est := RoundTo(RoundTo((grid.Cell[5, i].asfloat/100),-2) / 100 * SQL_Exec.FieldByName('imposto_estadual').AsFloat, -2);
                  end
                  else
                  begin
                    Imposto_Est := 0;
                ...
                vTotTrib := Imposto_Est;
                Total_Imp_Importacao := Total_Imp_Importacao + Imposto_Imp;
                Total_Imp_Nac := Total_Imp_Nac + Imposto_Nac;
                Total_Imp_Est := Total_Imp_Est + Imposto_Est;
        ...
        Total.ICMSTot.vTotTrib := Total_Imp_Est;
        ...
        InfAdic.infAdFisco := '';
    {    InfAdic.infAdFisco :=
        ' Informação dos Tributos Totais Incidentes (Lei Federal 12.741/2012): FONTE: IBPT (';
        InfAdic.infAdFisco := InfAdic.infAdFisco + ' ' + 'Federal  :  ' + FormatFloat('#,##0.00',Total_Imp_Importacao);
        InfAdic.infAdFisco := InfAdic.infAdFisco + ' ' + 'Nacionnal :  ' +FormatFloat('#,##0.00', Total_Imp_Nac);
        InfAdic.infAdFisco := InfAdic.infAdFisco + ' ' + 'Estadual:  ' + FormatFloat('#,##0.00',Total_Imp_Est);}
      end;
...
begin
  with frmmodulo do
  begin
//      MemoDados.text := ACBRNFCe.GetURLQRCode(ACBRNFCe.NotasFiscais.Items[0]
//      .NFe.Ide.cUF, ACBRNFCe.NotasFiscais.Items[0].NFe.Ide.tpAmb,
...
//      ACBRNFCe.NotasFiscais.Items[0].NFe.signature.DigestValue, edtIdToken,
//      edtTokenNumero);
  end;
end;
...
begin
  ...
  begin
    try
      ...
      if vContingencia=1 then
      begin
        ACBRNFCe.NotasFiscais.GerarNFe;
        ACBRNFCe.NotasFiscais.Assinar;
        ACBRNFCe.NotasFiscais.Validar;
        ACBRNFCe.NotasFiscais.Imprimir;
        Application.messagebox( ' NFC-e emitida em contigência , outra via vai sera impressa para conferencia!','CONTIGÊNCIA',MB_OK+MB_ICONINFORMATION);
        ACBRNFCe.NotasFiscais.Imprimir;
        cStatus := 100;
      ...
      on e: exception do
      begin
        application.messagebox(pwidechar('Erro na geração da NFCE' +
          'Erro: ' + e.Message), 'Erro', mb_ok + MB_ICONERROR);
        nfce_autorizada := false;
      end;
...
begin
  ...
  begin
    ...
    if formatfloat('##########0.00', rProd_total) <>
      formatfloat('##########0.00', ed_total_item.value) then
    begin
      if bTruncar_valor then
      begin
...
begin
  if frmPrincipal.TipoImpressora = SemImpressora then
    frmVenda.Color := $00404040
  else
    frmVenda.Color := $00134017;
end;
...
begin
  ...
  frmMsg_Operador.lb_msg.caption := 'Aguarde! Verificando status do Cupom NFCe...';
