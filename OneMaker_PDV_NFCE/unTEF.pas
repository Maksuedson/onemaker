...
var
  ...
  iTEFProximoNSU: integer = 0;
  // Nº sequencial único da transação gerado pelo TEFDelphi
  iTEFTecban: integer = 0;
  // 0 = Usar o GP das redes - 1 = Usar o GP Tecban / 2 Hipercard
  sTEFPath: string = '\TEF_DIAL\';
  sTEFTemp_Path: string = '.\TEF_DIAL\';
  sTEFRetorno: string;
  sTEFResposta: string;
...
uses principal, modulo, unECF, funcoes, venda, UFuncoes;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        CopyFile(pwidechar(sTEFTemp_Path + 'IntPos.001'),
          pwidechar(sTEFPath + 'RESP\IntPos.001'), false);
      end;
...
begin
  ...
  if (sTEFRetorno <> '0') and (sExibeErro = 'S') then
    Application.messagebox('Não existe nenhuma operação pendente!', 'Atenção!',
  ...
  try
    ...
    try
      ...
      begin
        ...
        repeat
          CopyFile(pwidechar(sTEFPath + 'RESP\IntPos.001'),
            pwidechar(sTEFTemp_Path + 'IntPos.001'), false);
            ...
            repeat
              ...
              begin
                ...
                begin
                  ...
                  if (sParcela <> '') and (sVencParcela <> '') and
                    (sValorParcela <> '') and (sNSUParcela <> '') then
                    WriteLn(TEFParc, sParcela + sVencParcela + sValorParcela +
                      sNSUParcela);
                end;
                Close(TEFParc);
              end;
    ...
    begin
      ...
      if sTEFValorTotal <> '' then
        sValorRet := 'Valor: ' + formatfloat('###,###,##0.00',
          StrToInt(sTEFValorTotal) / 100);
      ...
      begin
        ...
        begin
          ...
          texto_msg := pansichar('' + #13#10 + '' + #13#10 + '' + #13#10 +
            sTEFMensOperador);
          Application.messagebox(pwidechar(texto_msg), 'Atenção',
            mb_ok + MB_ICONWARNING);
        end
        else
        begin
          texto_msg := pansichar(sRedeRet + #13#10 + sDoctoRet + #13#10 +
            sValorRet + #13#10 + sTEFMensOperador);
...
begin
  ...
  try
    ...
    begin
      ...
      if (sOperacao = 'CHQ') or (sOperacao = 'CRT') or (sOperacao = 'CNC') or
        (sOperacao = 'CNF') or (sOperacao = 'NCN') then
        if Trim(sTEFDoctoVinculado) <> '' then
          WriteLn(IntPosTmp, ('002-000 = ' + sTEFDoctoVinculado));

      if (sOperacao = 'CHQ') or (sOperacao = 'CRT') or (sOperacao = 'CNC') then
      ...
      if (sOperacao = 'CHQ') or (sOperacao = 'CNC') then
      begin
      ...
      if (sOperacao = 'CNC') or (sOperacao = 'CNF') or (sOperacao = 'NCN') then
      begin
      ...
      Close(IntPosTmp);
      bRet := CopyFile(pwidechar(sTEFPath + 'REQ\IntPos.tmp'),
        pwidechar(sTEFPath + 'REQ\IntPos.001'),True);
      DeleteFile(sTEFPath + 'REQ\IntPos.tmp');
      ...
      if (sTEFRetorno = '0') and ((sOperacao = 'ADM') or (sOperacao = 'CHQ') or
        (sOperacao = 'CRT') or (sOperacao = 'CNC')) then
  ...
  TRY
    ...
    try
      ...
      begin
        ...
        sNumero_Cupom :=
          Zerar(somenteNumero(cECF_COO_Nao_Fiscal(iecf_modelo)), 6);
        ...
        sMsg := cECF_Abre_CNFV(iecf_modelo, sForma_Temp,
          (StrToFloat(sTEFValorTotal) / 100), sTEFDoctoVinculado);
        if sMsg <> 'OK' then
        begin
          BlockInput(false);

          if Application.messagebox('Impressora não responde!' + #13 +
            'Tentar novamente?', 'Abertura CNFV', mb_yesno + MB_ICONWARNING) = idYes
          then
          begin
            BlockInput(true);
            Application.ProcessMessages;
        ...
        begin
          try
            ...
            begin
              ...
              begin
                ...
                spNao_Fiscal.parambyname('valor').asfloat :=
                  rValor_Total_Cartao;
                spNao_Fiscal.parambyname('COO').asstring := sNumero_Cupom;
                spNao_Fiscal.parambyname('GNF').asstring :=
                  cECF_Numero_Contador_Operacao_NF(iecf_modelo);
                spNao_Fiscal.parambyname('GRG').clear;
                spNao_Fiscal.parambyname('CDC').asstring :=
                  cECF_Numero_Contador_Comprovante_CD(iecf_modelo);
          ...
          Repeat
            ...
            begin
              ...
              begin
                ...
                if Application.messagebox('Impressora não responde!' + #13 +
                  'Tentar novamente?', 'Impressão CNFV',
          ...
          begin
            ...
            begin
              ...
              BlockInput(false);
              if Application.messagebox('Impressora não responde!' + #13 +
                'Tentar novamente?', 'Fechamento CNFV',
                mb_yesno + MB_ICONWARNING) = idYes then
              begin
                BlockInput(true);
                Application.ProcessMessages;
      ...
      begin
        ...
        repeat
          ...
          sNumero_Cupom :=
            Zerar(somenteNumero(cECF_Numero_Cupom(iecf_modelo)), 6);
          bContinua := true;
          bParar := true;
          // rodar o arquivo de retorno para imprimir o cnfv
          AssignFile(TEFImp, sTEFTemp_Path + 'TEF.Imp');
          ...
          Repeat
            ...
            begin
              ...
              begin
                ...
                BlockInput(false);
                if Application.messagebox('Impressora não responde!' + #13 +
                  'Tentar novamente?', 'Comprovante TEF',
                  mb_yesno + MB_ICONWARNING) = idYes then
                begin
                  BlockInput(true);
                  Application.ProcessMessages;
          ...
          begin
            repeat
              ...
              begin
                BlockInput(false);
                if Application.messagebox('Impressora não responde!' + #13 +
                  'Tentar novamente?', 'Fechamento do Gerencial',
                  mb_yesno + MB_ICONWARNING) = idYes then
                begin
                  BlockInput(true);
                  Application.ProcessMessages;
              ...
              begin
                try
                  ...
                  begin
                    ...
                    begin
                      spNao_Fiscal.Close;
                      spNao_Fiscal.parambyname('codigo').asstring :=
                        codifica_cupom;
                      spNao_Fiscal.parambyname('ecf').asstring := sECF_Serial;
                      spNao_Fiscal.parambyname('data').asdatetime :=
                        dData_Sistema;
                      spNao_Fiscal.parambyname('hora').Astime :=
                        strtotime(Copy(cECF_Data_Hora(iecf_modelo), 12, 8));
                      ...
                      spNao_Fiscal.parambyname('GNF').asstring :=
                        cECF_Numero_Contador_Operacao_NF(iecf_modelo);
                      spNao_Fiscal.parambyname('GRG').asstring :=
                        cECF_Numero_Contador_Relatorio_Gerencial(iecf_modelo);
...
begin
  ...
  try
    ...
    begin
      ...
      Close(IntPos001);
      if sValor <> '' then
        sTextoValor := 'Valor:' + formatfloat('###,###,##0.00',
          StrToInt(sValor) / 100)
      else
        sTextoValor := '';

      texto_msg := pansichar('Cancelada a transação:' + #13 + #10 +
        ('Rede: ' + Trim(sRede)) + #13 + #10 + ('Doc No: ' + Trim(sDocto)) + #13
        + #10 + sTextoValor);

      if (Trim(sRede) <> '') or (Trim(sDocto) <> '') or (Trim(sTextoValor) <> '')
      then
      begin
        BlockInput(false);
        Application.messagebox(pwidechar(texto_msg), 'Atenção', mb_ok + MB_ICONWARNING);
...
begin
  ...
  try
    ...
    begin
      ...
      repeat
        Application.messagebox('Gerenciador Padrão não está ativo!', 'Atenção',
          mb_ok + MB_ICONWARNING);
        TEFGravaOperacao('ATV'); // 4 passo
      until sTEFRetorno = '0';
