...
//                      QRCAIXA_MOV.Params.ParamByName('VALOR').ASFLOAT := qrpdv.fieldbyname('valor_total').asfloat;
          ...
          // S E R V I D O R   PARA  E S T A C A O
              ...
              try
                {$region'Tabela dos Dados da Empresa' }
                qrServidor.CLOSE;
                qrServidor.SQL.CLEAR;
                qrServidor.SQL.Add('select * from C000004 ');
                qrServidor.OPEN;
                qrpdv.close;
                qrpdv.sql.clear;
                qrpdv.sql.add('select * from C000004 ');
                qrpdv.open;
                if qrpdv.RecordCount > 0 then
                qrpdv.edit else qrpdv.Insert ;

                for n := 0 to qrServidor.FieldDefs.Count -1 do
                  begin
                    if qrPDV.FindField(qrServidor.FieldDefs[n].Name)<>nil then
                    qrPDV.FieldByName(qrServidor.FieldDefs[n].Name).Value :=  qrServidor.FieldByName(qrServidor.FieldDefs[n].Name).Value;
                  end;
                qrpdv.Post;
                {$ENDREGION}

                {$region'Tabela dos Dados da C999999' }
                qrServidor.CLOSE;
                qrServidor.SQL.CLEAR;
                qrServidor.SQL.Add('select * from C999999 ');
                qrServidor.OPEN;
                qrpdv.close;
                qrpdv.sql.clear;
                qrpdv.sql.add('select * from C999999 ');
                qrpdv.open;
                if qrpdv.RecordCount > 0 then
                qrpdv.edit else qrpdv.Insert ;
                ...
                {$region'Tabela dos Dados da C000001' }
                qrServidor.CLOSE;
                qrServidor.SQL.CLEAR;
                qrServidor.SQL.Add('select * from C000001 ');
                qrServidor.OPEN;
                qrpdv.close;
                qrpdv.sql.clear;
                qrpdv.sql.add('select * from C000001 ');
                qrpdv.open;
                if qrpdv.RecordCount > 0 then
                qrpdv.edit else qrpdv.Insert ;
                      ...
                      begin
                        ...
                        begin
                          try
                            ...
                            qrpdv.parambyname('NCM').asstring := qrservidor_tabela.fieldbyname('NCM').asstring;
