  ...
  BEGIN
      ...
      BEGIN
        ...
        begin
          ...
          if application.messagebox(pwidechar('Cliente com crédito em Observação!, Deseja continuar?'),
            'Atenção', mb_yesno + MB_ICONERROR) = idno then
          begin
            if not frmPrincipal.autentica('Liberar venda',4) then
            begin
     ...
     if FRMMODULO.qrcliente.FIELDBYNAME('LIMITE').ASFLOAT <> 0 then
      begin
        ...
        BEGIN
          if application.messagebox(pwidechar('Este cliente possue um limite disponível de:'+#13+' ---> R$ '+lcliente_limite.caption+'  <--- '+#13+'É necessário autorização para prosseguir! '+#13+'Deseja continuar?'),
            'Atenção', mb_yesno + MB_ICONERROR) = idno then
            begin
              if not frmPrincipal.autentica('Liberar venda',4) then
              begin
