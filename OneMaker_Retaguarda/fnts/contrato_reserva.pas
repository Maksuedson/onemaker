...
begin
  ...
  // pessoa fisica
    fxcontrato.LoadFromFile('\OneMaker\server\rel\contrato_reserva.fr3')
  else
  begin
    // pessoa juridica
    if qrcliente.FieldByName('resp2_nome').asstring <> '' then
    // dois responsaveis
      fxcontrato.LoadFromFile('\OneMaker\server\rel\contrato_reserva_j2.fr3')
    else
    // um responsavel
      fxcontrato.LoadFromFile('\OneMaker\server\rel\contrato_reserva_j1.fr3')
...
begin
  arquivo_ini := TIniFile.Create('c:\OneMaker\server\dll\contrato.ini');
