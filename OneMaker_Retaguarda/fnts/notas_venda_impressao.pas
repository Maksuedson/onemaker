...
procedure Tfrmnotas_venda_impressao.AdvSmoothExpanderPanel1MouseDown(
...
06 - ---
07 - Contrato | Guia de Entrega | Carnê
08 - Personalizado
09 - Papel A4 - Laser / Tinta
10 - Papel Razão - Laser / Tinta
11 - Formulário Contínuo - Razão 2
12 - Contrato | Guia de Entrega | Carnê + Capa
13 - Bobina sem formatação
14 - Pedido + Carnê
15 - Contrato/Carnê
16 - Pedido + Carnê com código de barras
17 - Papel Razão - Laser / Tinta 2
}
{4 - 0 -04 - Formulário Contínuo - Razão
7 -1 Contrato | Guia de Entrega | Carnê
14 -2  Pedido + Carnê
16 -3 Pedido + Carnê com código de barras
17 -4 Papel Razão - Laser / Tinta 2  }
...
10 - Papel Razão - Laser / Tinta
  ...
  //  0: TPCupom_FiscalClick(frmnotas_venda_impressao);   1
   // 1: TPNota_FiscalClick(frmnotas_venda_impressao);    2
  //  2: TPBobinaClick(frmnotas_venda_impressao);         3
    0: TPRazaoClick(frmnotas_venda_impressao);
    //0: TPCartaClick(frmnotas_venda_impressao);          5
    //5: TPImpressoClick(frmnotas_venda_impressao);       6
    1: TPContratoClick(frmnotas_venda_impressao);
    //: TPPersonalizadoClick(frmnotas_venda_impressao);  8
    //8: TPA4Click(frmnotas_venda_impressao);             9
    //9: TPRazaoA4Click(frmnotas_venda_impressao);        10
    //10: FormulrioContnuoRazo1click(frmnotas_venda_impressao);  11
//  11: nil;                                                   12
    //12: Bobinasemformatao1Click(frmnotas_venda_impressao);     13
    2: PedidoCarn1Click(frmnotas_venda_impressao);
//  14 : //                                                    15
    3: PedidoCarncomcdigodebarras1Click(frmnotas_venda_impressao);
    4: Laser21Click(frmnotas_venda_impressao);
...
begin
  ...
  FXvenda.LoadFromFile('c:\OneMaker\server\rel\f000110.fr3');
  FXvenda.ShowReport;
end;
...
begin
  ...
  begin
    ...
    frmmodulo.qrrelatorio.post;
    FXvenda.LoadFromFile('c:\OneMaker\server\rel\f000074.fr3');
    FXvenda.ShowReport;
    FXvenda.LoadFromFile('c:\OneMaker\server\rel\f000153.fr3');
    FXvenda.ShowReport;
  ...
  begin
    ...
    frmmodulo.qrrelatorio.fieldbyname('linha6').asstring := IntToStr(QRPRODUTO_MOV.RecordCount);
    frmmodulo.qrrelatorio.post;
    FXvenda.LoadFromFile('c:\OneMaker\server\rel\f000050.fr3');
    FXvenda.ShowReport;
...
//    frmmodulo.qrrelatorio.fieldbyname('linha7').asstring :=
  ...
  if QRCONTASRECEBER.RECORDCOUNT > 0 then
  begin
    FXvenda.LoadFromFile('c:\OneMaker\server\rel\f000049.fr3');
    FXvenda.ShowReport;
  end;
...
begin
  ...
  frmmodulo.qrrelatorio.fieldbyname('linha6').asstring := IntToStr(QRPRODUTO_MOV.RecordCount);
  frmmodulo.qrrelatorio.post;
  FXvenda.LoadFromFile('c:\OneMaker\server\rel\f000154.fr3');
  FXvenda.ShowReport;
  if QRCONTASRECEBER.RECORDCOUNT > 0 then
  begin
    FXvenda.LoadFromFile('c:\OneMaker\server\rel\f000153.fr3');
...
//    frmmodulo.qrrelatorio.fieldbyname('linha7').asstring := nome_condicao;
  ...
  frmmodulo.qrrelatorio.post;
  FXvenda.LoadFromFile('c:\OneMaker\server\rel\f000155.fr3');
  fxvenda.ShowReport;
end;
