...
type
  Tfrmdados_inventario = class(TForm)
    ...
    procedure eert00087Click(Sender: TObject);
  private
...
begin
  ...
  if FileExists('\OneMaker\server\bd\eDI.inf') then
  begin
    assignfile(txt, '\OneMaker\server\bd\eDI.inf');
    reset(txt);
...
procedure Tfrmdados_inventario.eert00087Click(Sender: TObject);
begin
...
begin
  ...
  frminventario.fXproduto.LoadFromFile('\OneMaker\server\rel\f000126.fr3');
