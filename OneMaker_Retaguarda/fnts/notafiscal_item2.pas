...
type
  TfrmNotaFiscal_Item2 = class(TForm)
    ...
    Label39: TLabel;
    Label40: TLabel;
    ed_frete: TRzNumericEdit;
    Label41: TLabel;
    ed_aliquota_cred_icms: TRzNumericEdit;
    Label42: TLabel;
    edt_cred_valor: TRzNumericEdit;
...
begin
  ...
  frmmodulo.qrproduto.SQL.Add('select * from c000025 where CODIGO = :CODIGO');
  frmmodulo.qrproduto.ParamByName('CODIGO').asstring :=  ed_codigo.text;
  frmmodulo.qrproduto.open;

  if  (frmmodulo.qrproduto.fieldbyname('CLASSIFICACAO_FISCAL').asstring <>  ed_classific.text) or
       (frmmodulo.qrproduto.fieldbyname('cest').asstring <>  ed_cest.text) then
  begin
     frmmodulo.qrproduto.Edit;
     frmmodulo.qrproduto.fieldbyname('CLASSIFICACAO_FISCAL').asstring := ed_classific.text;
     frmmodulo.qrproduto.fieldbyname('cest').asstring :=  ed_cest.text;
  ...
  begin
    ...
    FrmNotaFiscal.qrnotafiscal_item.sql.add('CLASSIFICACAO_FISCAL,CSOSN,CST,VALOR_ICMS,');
    ...
    FrmNotaFiscal.qrnotafiscal_item.sql.add('pis_base,pis_valor,pis_aliquota,cofins_base,cofins_valor,cofins_aliquota,codbarra,OUTRAS, origem, cest, cst_pis, cst_cofins, piscofins, freteitem, CREDSN_ALIQUOTA, CREDSN_VALOR   )');
    ...
    FrmNotaFiscal.qrnotafiscal_item.sql.add(':BASE_SUB,:ICMS_SUB,:ISENTO,:CODLANCAMENTO,');
    FrmNotaFiscal.qrnotafiscal_item.sql.add(':DESCONTO,:SUBTOTAL,:PESO_BRUTO,:PESO_LIQUIDO,:ITEM,:codigo,:grade,:complemento,');
    FrmNotaFiscal.qrnotafiscal_item.sql.add(':pis_base,:pis_valor,:pis_aliquota,:cofins_base,:cofins_valor,:cofins_aliquota,:cbarra,:OUTRAS, :origem, :cest, :cst_pis, :cst_cofins, :piscofins, :freteitem, :CREDSN_ALIQUOTA, :CREDSN_VALOR )');
    ...
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('ORIGEM').asfloat := cb_Origem.ItemIndex;
    ...
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('cst_pis').asstring := trim(qrproduto.fieldbyname('cst_pis').asstring);
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('cst_cofins').asstring := trim(qrproduto.fieldbyname('cst_cofins').asstring);
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('piscofins').asstring := qrproduto.fieldbyname('piscofins').asstring;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('freteitem').asfloat := ed_frete.Value;

     FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('CREDSN_ALIQUOTA').asfloat := ed_aliquota_cred_icms.Value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('CREDSN_VALOR').asfloat := edt_cred_valor.Value;
  ...
  begin
    ...
    FrmNotaFiscal.qrnotafiscal_item.sql.add('cst_pis             = :cst_pis,');
    FrmNotaFiscal.qrnotafiscal_item.sql.add('cst_cofins            = :cst_cofins,');
    FrmNotaFiscal.qrnotafiscal_item.sql.add('piscofins         = :piscofins,');
    FrmNotaFiscal.qrnotafiscal_item.sql.add('freteitem         = :freteitem,');


    FrmNotaFiscal.qrnotafiscal_item.sql.add('CREDSN_ALIQUOTA            = :CREDSN_ALIQUOTA,');
    FrmNotaFiscal.qrnotafiscal_item.sql.add('CREDSN_VALOR         = :CREDSN_VALOR,');
    ...
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('ISENTO').asfloat := ed_isenta.value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('OUTRAS').asfloat := ed_outras.value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('origem').AsInteger :=  cb_Origem.ItemIndex;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('cest').asstring := ed_cest.Text;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('DESCONTO').asfloat := ed_desconto.value;
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('SUBTOTAL').asfloat := ed_subtotal.value;
  ...
  begin
    vtot_produtos := frmnotafiscal_menu.qrnota.FieldByName('VALOR_PRODUTOS').asfloat + (ed_subtotal.value);
    frmnotafiscal_menu.qrnota.FieldByName('VALOR_PRODUTOS').asfloat := vtot_produtos;
  end
  else
  begin
    vtot_produtos := frmnotafiscal_menu.qrnota.FieldByName('VALOR_PRODUTOS').asfloat + (ed_total.value - ed_desconto.value);
    frmnotafiscal_menu.qrnota.FieldByName('VALOR_PRODUTOS').asfloat := vtot_produtos;
  end;
...
procedure TfrmNotaFiscal_Item2.FormCreate(Sender: TObject);
begin
  itm:=0;
end;
  ...
  begin
    ...
    begin
      ...
      begin
           ...
           ed_icms_reducao.value := query1.fieldbyname('reducao_icms').Asfloat;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                         ed_icms_base.value := ((ed_total.value * 0.07)* 100) / ed_icms_aliquota.Value;
                    ed_icms_valor.value := ed_icms_base.value * (ed_icms_aliquota.Value / 100);
                    ed_sub_base.value := ((((ed_total.value * (ed_margem_agregada.Value / 100)) + ed_total.value) * 0.07)* 100) / ed_icms_aliquota.Value;
                    ed_sub_valor.value := ed_sub_base.value * (ed_icms_aliquota.value / 100) - ed_icms_valor.value;
              end
              else
              begin
                ed_sub_base.value := 0;
    ...
    begin
      ...
      begin
        ...
        begin
          if frmmodulo.qrFilial.FieldByName('SUBSTITUTO_TRIBUTARIO').AsString = 'S' then
          begin
            ed_sub_base.value := ed_total.Value + (((ed_total.value + ed_ipi_valor.Value + ed_pis_valor.Value + ed_cofins_valor.Value)* ed_margem_agregada.Value / 100) -
                                                  (ed_icms_base.value * ed_icms_reducao.value / 100));
            ed_sub_valor.value := ((ed_sub_base.value * ed_icms_aliquota.value / 100)- ed_icms_valor.value);
            ed_icms_base.value := ed_total.value;
            ed_icms_valor.value := ed_total.value *(ed_icms_aliquota.value / 100);
      ...
      begin
        ...
        begin
          if (frmmodulo.qrFilial.FieldByName('SUBSTITUTO_TRIBUTARIO').AsString = 'S') then
          begin
            ed_sub_base.value := ed_total.value +((ed_total.value + ed_ipi_valor.Value +
                                 ed_pis_valor.Value + ed_cofins_valor.Value)*
                                 (ed_margem_agregada.value / 100)) ;
          ...
          begin
            ...
            begin
              ...
              begin
                ed_icms_base.value := ed_total.value;
                ed_icms_valor.value := ed_total.value *(ed_icms_aliquota.value / 100);
                ed_sub_base.value := ed_icms_base.value +(ed_total.value * (ed_margem_agregada.value / 100)) ;
                ed_sub_valor.value := ed_sub_base.value * (ed_icms_aliquota.value / 100)- ed_icms_valor.value;
