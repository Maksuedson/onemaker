unit prevenda_entrega;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, DB, DBClient, ZAbstractRODataset, ZAbstractDataset, ZDataset,
  frxClass, frxDBSet, frxDesgn, Menus, StdCtrls, Spin, Mask, wwdbedit,
  AdvGlowButton, ExtCtrls, Grids, Wwdbigrd, Wwdbgrid;

type
  TfrmPrevenda_Entrega = class(TForm)
    wwDBGrid1: TwwDBGrid;
    Bevel1: TBevel;
    Panel1: TPanel;
...
var
  frmPrevenda_Entrega: TfrmPrevenda_Entrega;
  NAO_incluir : boolean;
...
uses modulo, principal, prevenda, prevenda_fechamento;

{$R *.dfm}

procedure TfrmPrevenda_Entrega.FormClose(Sender: TObject;
...
begin
     ...
     begin
       ...
       qrentrega2.sql.add('(:CODVENDA,:CODPRODUTO,:QTDE_ENTREGUE,:qtde_vendida,:codcliente,:data_venda,:data_entrega)');
       qrentrega2.Params.ParamByName('codvenda').asstring     := numero_vendax;
       ...
       qrentrega2.params.parambyname('codcliente').asstring := FRMMODULO.QRCLIENTE.FIELDBYNAME('CODIGO').ASSTRING;
       qrentrega2.params.ParamByName('data_venda').asdatetime := frmmodulo.qrcaixa_operador.FIELDBYNAME('DATA').ASdatetime;
       qrentrega2.Params.ParamByName('data_entrega').asdatetime := frmmodulo.qrcaixa_operador.FIELDBYNAME('DATA').ASdatetime;
       qrentrega2.ExecSQL;
       qrentrega.Next;
     end;
  NAO_incluir := FALSE;
...
procedure TfrmPrevenda_Entrega.bcancelarClick(Sender: TObject);
begin
  close;
end;

procedure TfrmPrevenda_Entrega.Cancelar1Click(Sender: TObject);
begin
  close;
end;

procedure TfrmPrevenda_Entrega.bgravarClick(Sender: TObject);
begin
     ...
     0 : bt_bobinaClick(frmprevenda_entrega);
     1 : bt_razaoClick(frmprevenda_entrega);
     2 : bt_cartaClick(frmprevenda_entrega);
     3 : bt_laserClick(frmprevenda_entrega);
     end;
  close;
end;

procedure TfrmPrevenda_Entrega.FormShow(Sender: TObject);
begin
  NAO_incluir := TRUE;
...
procedure TfrmPrevenda_Entrega.qrEntregaBeforeInsert(DataSet: TDataSet);
begin
  if NAO_incluir then abort;
end;

procedure TfrmPrevenda_Entrega.ed_vendidoExit(Sender: TObject);
begin
qrEntrega.fieldbyname('saldo')
end;

procedure TfrmPrevenda_Entrega.ed_entregueExit(Sender: TObject);
begin
...
procedure TfrmPrevenda_Entrega.bt_razaoClick(Sender: TObject);
...
// impressao 10 cpi     --> #27#80
   ...
   writeln(arquivo,#27#80+#18+#27#71+#20,frmprincipal.Texto_Justifica(emitente_fantasia,12,' ','D')+#27#80+#15+#27#72+#20+' '+frmprincipal.Texto_Justifica(emitente_telefone,13,' ','D')+
                   #27#80+#18+#27#72+#20+' GUIA DE ENTREGA --> VENDA NR.: '+#27#80+#18+#27#71+#20+frmprincipal.Texto_Justifica(numero_vendax,6,'0','E')+
...
procedure TfrmPrevenda_Entrega.bt_bobinaClick(Sender: TObject);
VAR arquivo : textfile;
    I, VIAS : integer;
    SUBIR_PAPEL : integer;
begin
...
// impressao 10 cpi     --> #27#80
   ...
   writeln(arquivo,#15+#27#72+#20,'GUIA DE ENTREGA - No. '+frmprincipal.Texto_Justifica(numero_vendax,6,'0','E')+
                                  '  Emis: '+frmprincipal.Texto_Justifica(DateToStr(frmmodulo.qrcaixa_operador.FIELDBYNAME('DATA').ASdatetime),10,' ','E')+
                                  ' '+frmprincipal.Texto_Justifica(timetostr(time),5,' ','E'));
   writeln(arquivo,#15+#27#72+#20,'----------------------------------------------------');
   writeln(arquivo,#15+#27#72+#20,'Cliente...: '+frmprincipal.Texto_Justifica(frmmodulo.qrcliente.fieldbyname('nome').asstring,6,'0','E')+' '+frmprincipal.Texto_Justifica(frmmodulo.qrcliente.fieldbyname('nome').asstring,32,' ','D'));
...
procedure TfrmPrevenda_Entrega.bt_cartaClick(Sender: TObject);
VAR arquivo : textfile;
    I, VIAS : integer;
begin
// NORMAL               --> #18+#27#72+#20
...
procedure TfrmPrevenda_Entrega.bt_laserClick(Sender: TObject);
begin
  frmmodulo.qrrelatorio.open;
  frmmodulo.qrrelatorio.edit;
  frmmodulo.qrrelatorio.fieldbyname('linha10').asstring := numero_vendax;
  frmmodulo.qrrelatorio.FieldByName('linha6').asstring := DateToStr(frmmodulo.qrcaixa_operador.FIELDBYNAME('DATA').ASdatetime);
  frmmodulo.qrrelatorio.FieldByName('linha9').asstring := frmmodulo.qrcaixa_operador.FIELDBYNAME('CODIGO').ASSTRING;
  frmmodulo.qrrelatorio.FieldByName('linha8').asstring := frmprevenda.qrorcamento.fieldbyname('codvendedor').asstring;

  qrEntrega.filtered := false;
  qrentrega.filter := 'qtde_entregue > 0';
  qrentrega.Filtered := true;

  rel_entrega.Report.LoadFromFile('\siscom\server\rel\F000048_2.fr3');
